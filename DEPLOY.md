...
> **Put the service in the same region as the database.** Every request that
> touches Postgres pays the round trip. Measured on this project, an Oregon
> service against a `us-east-2` database added **~60–80ms per request** — the
> response cache hides it from repeat visitors but not from the first one.
> Render's region is immutable after creation, so getting it wrong means
...
| `USER_AGENT` | **a real contact address** — the SEC requires one on automated requests and the RSS sources expect it; a placeholder is what gets a project blocked |
